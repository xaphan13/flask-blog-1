...
Контекст-инструкция для AI-агентов, работающих с кодом в этом репозитории.
Файл не привязан к конкретному харнесу; `QWEN.md` — копия тех же указаний для Qwen Code,
**держите оба файла синхронными.**

🇬🇧 English version: [AGENTS.md](AGENTS.md)
Пользовательская документация: [README.ru.md](README.ru.md) ·
Отчёт по настройке: [`docs/setup-and-run.md`](docs/setup-and-run.md)
...
Требуются `flaskblog/dock_flask.env`, файл `.env` рядом с compose-файлом и self-signed
сертификаты в `nginx/cert/` — всё это в `.gitignore`; см. `docs/setup-and-run.md` §5.2.
Доступ: <https://localhost:1443/>; pgAdmin на `/pgadmin`; Postgres опубликован на
`127.0.0.1:9032`.
...
модуля и называются по имени блюпринта (`main`, `users`, `errors`, `art_main`);
перенаправления между блюпринтами используют форму эндпоинта
...
- Добавить статью = добавить запись `ArticleLang(...)` в `art_files` в
  ...
  `templates/content_art/<file>.html`. Словарь `art_dict_file` — это comprehension по
  `art_files`, индексированный по `art_id`; именно его ищет маршрут
  `/art/<author>/<art_id>`.
