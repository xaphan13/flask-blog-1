...
> Проверено на коммите `aa763d0`. Таблица маршрутов и коды ответов получены прогоном
> `app.test_client()`, а не чтением кода. Архитектурный контекст: [02_architecture.md](02_architecture.md).
...
1. **`author` игнорируется** — используется только в логе. Поиск идёт исключительно по
   `art_id`, поэтому `/art/anybody-else/1` возвращает ту же статью с кодом 200 (проверено).
2. `art_dict_file[art_id]` — прямое индексирование словаря. Отсутствующий ключ даёт
   `KeyError`, который превращается в **500, а не в 404** (проверено на `/art/Max/999`).
   Корректный вариант — `art_dict_file.get(art_id)` с `abort(404)`.
3. `read_html(art.file_name)` открывает файл в каталоге, зафиксированном при импорте,
   и читает его целиком. Кэша нет — файл читается на каждый запрос.
   Отсутствующий файл даст `FileNotFoundError` → тоже 500.
4. `art.content = content` — **мутация объекта, разделяемого между всеми запросами.**
   Сейчас безопасно, потому что записывается одно и то же значение; любое
   запросо-зависимое поле здесь немедленно станет источником гонки.
5. Шаблон выводит `{{ art.content|safe }}` — экранирование отключено сознательно,
   так как в файлах лежит HTML-разметка. Контур доверия: содержимое `content_art/`
   приравнивается к исходному коду.
